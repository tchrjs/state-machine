...
export class StateMachine {
    ...
    constructor(states: Array<State>, initial_state: State) {
    ...
    public transition(state: State, new_state_id: number | string) {
        // If transition called is not of current state, do nothing.
        if (this.current_state !== state) {
            return;
        }

        // Get and enter new state.
        let new_state: State | undefined = this.states[new_state_id];
        if (new_state === undefined) {
            console.error(`State '${new_state_id}' does not exist.`);
            return;
        } else {
            this.enterState(new_state);
        }
    }
